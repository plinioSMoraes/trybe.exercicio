...
const studentsCount = (objects, prof)=> {
  let objSize = Object.keys(objects).length;
  let numeroEstudantes = 0;
  let keyArr = Object.keys(objects); 
  for (let index = 0; index < objSize; index += 1) {
    if (objects[keyArr[index]].professor === prof) {
      numeroEstudantes += objects[keyArr[index]].numeroEstudantes; 
    }
  }
  console.log(numeroEstudantes);
}
// studentsCount(allLessons(), 'Maria Clara');
